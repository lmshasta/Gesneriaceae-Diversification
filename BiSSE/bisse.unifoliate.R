...
trees<-read.tree("ow.trees.tre")

#import dataset
data=read.table("data.unifoliate.ow.txt", row.names=1)
uni<-data[,1]
names(uni)<-row.names(data)

#skeletal tree approach for random sampling
sampling.f<-c(0.36,0.25)

#full bisse model
fit<-lapply(1:length(pruned.trees), function(x){
  model<-make.bisse(tree=pruned.trees[[x]], states=uni, sampling.f=sampling.f)
...
#mcmc - equal extinction
  model<-make.bisse(tree=pruned.trees[[1]],states=uni,sampling.f=sampling.f)
...
#best model for uniphytism was with equal extinction (mu1 ~ mu0) 
#mcmc - equal extinction
fit<-lapply(1:length(random.trees),function(x){
  model<-make.bisse(tree=random.trees[[x]],states=uni,sampling.f=sampling.f)
  lik<-constrain(model,lambda1 ~ lambda0)
  #start.values<-starting.point.bisse(pruned.trees[[1]])
  #res<-find.mle(model,x.init=start.values[argnames(lik)],method="subplex",control=list(maxit=50000))
  set.seed(1)
  #prior<-make.prior.exponential(1/(2*(start.values[2]-start.values[3])))
  samples<-mcmc(lik,start.values,nsteps=10000,w=w,prior=prior)
  cat("job #",x,"finished\n",sep="")
  return(samples)
})

fit.p<-as.data.frame(do.call("cbind",lapply(fit,function(x)( c(x$p)))))

#write output to file
output<-paste(Sys.getenv("HOME"),"/fit.p.txt",sep='')
write.table(fit.p,file=output,row.names=F,quote=F,sep="\t")
