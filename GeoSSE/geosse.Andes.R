...
data=read.table("data.andes.txt", row.names=1)
andes<-data[,1]
names(andes)<-row.names(data)

#skeletal tree approach for random sampling
sampling.f<-c(0.50,0.25,0.25)

#full geosse model
fit<-lapply(1:length(pruned.trees), function(x){
  model<-make.geosse(tree=pruned.trees[[x]], states=andes, sampling.f=sampling.f)
