...
data=read.table("data.pacific.txt", row.names=1)
pacific<-data[,1]
names(pacific)<-row.names(data)

#skeletal tree approach for random sampling
sampling.f<-c(0.25,0.25,0.25)

#full bisse model
fit<-lapply(1:length(pruned.trees), function(x){
  model<-make.geosse(tree=pruned.trees[[x]], states=pacific, sampling.f=sampling.f)
